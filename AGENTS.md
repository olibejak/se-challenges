...
Replace `packages/hardhat/contracts/Verifier.sol` with the full `HonkVerifier` contract
generated in Checkpoint 4 at `packages/circuits/target/Verifier.sol`.
Do this by overwriting the file contents
(e.g. `cp packages/circuits/target/Verifier.sol packages/hardhat/contracts/Verifier.sol`).
...
Hardhat package and there is no workaround.
